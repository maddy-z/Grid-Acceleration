...
Vec3f RayTracer::traceRayStat(Ray & ray, float tmin, int bounces, float weight, float indexOfRefraction, Hit & hit, bool noshadow_flag) const
{
	...
	RayTracing_Stats::IncrementNumNonShadowRays();
	...
	for (int i = 0; i < sparser -> getLightNum(); ++i) 
	{
		...
		Hit shadowHit(FLT_MAX, NULL, Vec3f(0.0f, 1.0f, 0.0f));
		Ray lightRay(dirToLight, hitPoint + EPSILON * dirToLight);

		// If shadow intersects with object

		if ( !noshadow_flag ) {
			RayTracing_Stats::IncrementNumShadowRays();
		}

		if ( !noshadow_flag && group -> intersect(lightRay, shadowHit, tmin) && shadowHit.getT() < lights[i] -> getDistance(hitPoint) ) {
	...
	if ( hit.getMaterial() -> isMirror() ) 
	{
		...
		Vec3f mirrorShade = traceRayStat ( mirrorRay, tmin, bounces + 1, weight, indexOfRefraction, mirrorHit, noshadow_flag );
		Vec3f mirrorColor = hitMaterial->getReflectiveColor();
	...
	if ( hit.getMaterial() -> isTransparent() )
	{
		...
		if ( hasTransmitLight )
		{
			...
			const Vec3f & transShade = traceRayStat ( transRay, tmin, bounces + 1, weight, indexOfRefraction, transHit, noshadow_flag );
			const Vec3f & transColor = hitMaterial->getTransparentColor();
...
Vec3f RayTracer::traceRayFast(Ray & ray, float tmin, int bounces, float weight, float indexOfRefraction, Hit & hit, 
							  bool noshadow_flag, const Grid & sceneGrid, Grid_Rendering_Type type) const

{
	...
	if ( type == RayTracer::NORMAL ) {
		if ( sceneGrid.intersectWithOverlapObject(ray, hit, tmin) == false )
			return sparser -> getBackgroundColor();
	}
	else if ( type == RayTracer::VISUAL_GRID ) {
		if ( sceneGrid.intersectWithGridVisualization(ray, hit, tmin) == false )
			return sparser -> getBackgroundColor();
	}
	else if ( type == RayTracer::VISUAL_GRID_COUNT ) {
		// TODO
	}
	...
	for (int i = 0; i < sparser -> getLightNum(); ++i) 
	{
		...
		if ( !noshadow_flag )																			// if enabling shadow casting
		{
			Ray lightRay(dirToLight, hitPoint + EPSILON * dirToLight);
			Hit shadowHit(FLT_MAX, NULL, Vec3f(0.0f, 1.0f, 0.0f));
				
			float dist = lights[i]->getDistance(hitPoint);

			if ( type == RayTracer::NORMAL && sceneGrid.intersectWithOverlapObject(lightRay, shadowHit, tmin) && shadowHit.getT() < dist ) {
				// RayTracing_Stats::IncrementNumIntersections();
				continue;
			} else if ( type == RayTracer::VISUAL_GRID && sceneGrid.intersectWithGridVisualization(lightRay, shadowHit, tmin) && shadowHit.getT() < dist ) {
				// RayTracing_Stats::IncrementNumIntersections();
				continue;
			} else if ( type == RayTracer::VISUAL_GRID_COUNT && sceneGrid.intersectWithGridVisualizationCount(lightRay, shadowHit, tmin) && shadowHit.getT() < dist ) {
				// RayTracing_Stats::IncrementNumIntersections();
				continue;
			}
		} 
	...
	if ( hit.getMaterial() -> isMirror() ) 
	{
		...
		Vec3f mirrorShade = traceRayFast ( mirrorRay, tmin, bounces + 1, weight, indexOfRefraction, mirrorHit, noshadow_flag, sceneGrid, type );
		Vec3f mirrorColor = hitMaterial->getReflectiveColor();
	...
	if ( hit.getMaterial() -> isTransparent() )
	{
		...
		if ( hasTransmitLight )
		{
			...
			const Vec3f & transShade = traceRayFast ( transRay, tmin, bounces + 1, weight, indexOfRefraction, transHit, noshadow_flag, sceneGrid, type );
			const Vec3f & transColor = hitMaterial->getTransparentColor();
