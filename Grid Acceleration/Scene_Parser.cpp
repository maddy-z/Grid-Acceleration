...
Grid * SceneParser::parseGrid()
{
	...
	Vec3f min_coor = readVec3f();
	getToken(token); assert (!strcmp(token, "max"));
	Vec3f max_coor = readVec3f();

	getToken(token); assert (!strcmp(token, "numCells"));
	Vec3f grid_size = readVec3f();
	
	getToken(token); assert (!strcmp(token, "rasterizeSphere"));
	Sphere * sphere = parseSphere();
	
	Grid * grid = new Grid(min_coor, max_coor, grid_size[0], grid_size[1], grid_size[2], currentMaterial);
	grid -> rasterizeSphere(sphere->getCenter(), sphere->getRadius());
	delete sphere;
