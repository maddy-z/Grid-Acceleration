...
public:
	...
	void Set_X(float d) { data[0] = d; }
	void Set_Y(float d) { data[1] = d; }
	void Set_Z(float d) { data[2] = d; }
...
public:
	...
	float Length() const {
		float l = (float)(sqrt( data[0] * data[0] + data[1] * data[1] + data[2] * data[2] + data[3] * data[3] ));
		return l;
	}
...
inline Vec3f calcNormal(const Vec3f & a, const Vec3f & b, const Vec3f & c)
{
	Vec3f n(0.0f, 0.0f, 1.0f);
	Vec3f::Cross3(n, b-a, c-a);
