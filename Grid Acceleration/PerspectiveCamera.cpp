...
#define		PI					3.1415926
#define		angleToFloat(a)		(a) / (180.0f) * PI
...
void PerspectiveCamera::initialize()
{
	...
	if (temp > 1.0e-6 || temp < (-1.0f) * 1.0e-6)                        // if orthonal to direction vector ~~~
	{                     
		...
		std::cout << "End adjusting Camera's Up Vector" << std::endl << std::endl;
